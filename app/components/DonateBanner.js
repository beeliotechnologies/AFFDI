  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-10 sm:px-6 md:py-14 lg:px-8">
        <div className="max-w-3xl rounded-2xl border border-white/60 bg-white/95 p-5 shadow-[0_18px_40px_rgba(9,14,88,0.14)] backdrop-blur-sm sm:p-6 md:p-9">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-[#090E58]">{eyebrow}</p>
            ...
            <ul className="mt-6 space-y-3 text-slate-700" aria-label="Reasons to donate">
              {points.map((point, index) => (
                <li key={index} className="flex items-start gap-3">
                  <span
                    className="mt-1 inline-block h-2.5 w-2.5 flex-shrink-0 rounded-full bg-[#090E58]"
                    aria-hidden="true"
                  />
          ...
          <a
            href={actionHref}
            className="mt-7 inline-flex rounded-xl bg-[#E5A223] px-5 py-2.5 text-sm font-bold text-[#090E58] transition hover:bg-[#C98C1D] sm:px-6 sm:py-3"
